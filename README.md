...
2. **Set the environment variables** in Vercel → Settings → Environment
   ...
   | `ADMIN_PIN` / `ADMIN_SECRET_KEY` | `ADMIN_PIN` is the first PIN, until it is changed from /admin. `ADMIN_SECRET_KEY` is the permanent recovery key. See Admin chrome. Do not hardcode a PIN. |
...
`/admin` is a PIN gate matching Createch Hobbies: the PIN is stored in
`sessionStorage`, sent as `X-Admin-Key`, plus an httpOnly session cookie.
Robots are told to skip `/admin`.

Which PIN works:

- **`ADMIN_SECRET_KEY`** always — the recovery key. Keep it long and somewhere
  safe; it is how you get back in if the PIN is forgotten.
- **The PIN set from /admin → Change PIN.** It is hashed and stored on Railway
  (`/api/admin/pin/`). Changing it signs out every other open session.
- **`ADMIN_PIN`** only until a PIN has been set from /admin. After that it is
  retired, so a stale value in the Vercel dashboard opens nothing.

If Railway is unreachable, only the recovery key works.
